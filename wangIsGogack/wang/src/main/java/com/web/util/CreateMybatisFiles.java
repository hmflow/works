...
public class CreateMybatisFiles {
     ...
     * @return  
     */
	@RequestMapping("/retore")
	public void restorMybatisInfo(@RequestParam Map sqlparam) { 
		// mappers/util/ADMIN01 의 findMapper쿼리를 호출합니다
		List<Map> qryData = dbConnService.selectDataSetList("com.web.mapperImplss.util.UtilMapper.findMapper", sqlparam);
	...
	private void createMapperAndXml(List<Map> qryData) {
		...
		// 파라메터로 받은 List<Map> 의 Map 의 mapperName 별로 분류합니다.
		// mapperName은 MAPPERS_PACKAGE_PATH 아래의 패키지명이 되어집니다.
		Map<Object, List<Map>> mapStream = qryData.stream().collect(Collectors.groupingBy(m->m.get("mapperName")));
		// 위에서 분류한 mapperName별로 분류한 Xml파일을 생성합니다.
	...
	public static void main(String[] args) {
		CreateMybatisFiles ccc= new CreateMybatisFiles();
		ccc.removeAllMapperAndXml();
	}
}
